...
Use the Low Birth Weight dataset (lowbwt.sav) to answer the following questions. The variable LOW is coded as 1 if the birth weight < 2500 grams (0 otherwise).
...
bwt |> 
  table1(~RACE | LOW, data = _, overall = FALSE, caption = "Low birthweight distribution")
...
glm(LOW ~ RACE, bwt, family = binomial) |> 
  modelsummary(
    ...
    statistic = "p={p.value}{stars} CI = [{conf.low}; {conf.high}]", 
    gof_omit = "RMSE|F|Log", 
    notes = "+ p<0.1, * p<0.05"
    )
...
```{r ex1-c-test, eval=FALSE, include=FALSE}
m0.glm <- bwt |> 
  glm(LOW.numeric ~ AGE, data = _, family = "binomial")
m0.lm <- bwt |> 
  lm(LOW.numeric ~ AGE, data = _)

modelsummary(list(m0.glm, m0.lm), stars = TRUE)
library(marginaleffects)
m0.glm |> avg_comparisons(variable = "AGE")
...
m1 <- bwt |> 
  glm(LOW.numeric ~ AGE + SMOKE, data = _, family = "binomial")

m2 <- bwt |> 
  glm(LOW.numeric ~ AGE + SMOKE + RACE, data = _, family = "binomial")

m3 <- bwt |> 
  glm(
    LOW.numeric ~ AGE + (SMOKE + RACE)^2, 
    data = _, 
    family = "binomial"
    )

modelsummary(
  title = "Modelling low birthweight", 
  list(m0, m1, m2, m3), 
  estimate = "{estimate} ({std.error})", 
...
**NOTE:** Notice that the first model allows us to interpret the effect of age on the rate of low-birthrate by recourse to odds ratio: every additional year in a mother's age increases her baby's odds by a factor of $\exp(0.051) = 1.052$ or about 5%. But it is hard to fully grasp what an increase of 5% in terms of odds actually means in terms of probabilities. 

To address this issue, we could either inspect the slope of the linear regression (which turns out to be 0.0104, suggesting that every year is associated with an increase in probability of about 0.0104), or use the `marginaleffects` package to find that the marginal effect of one additional year in a mother's age is associated with an additional 0.0107 in the probability of her baby being born with a low birth weight, as can be seen in the code below: 

```{r ex5-marginal, echo=TRUE}
library(marginaleffects)
m0 |> avg_comparisons(variable = "AGE") |> 
  as_tibble()

```

You thus have the option of saying either that the odds of having a low-birth weight increase by a factor of about 5% **or** that the probability increases by an additional 0.01 (on average) for every additional year in mother's age.  


(d) Is effect of smoking different for different RACE-groups?

**Answer**: On fitting a model with RACE, SMOKE and RACE*SMOKE (see the last model in the previous section) we find that the interaction is not significant. We do not have evidence for a different effect of smoking in different RACE-groups. We can also test the models with and without interactions using the likelihood ratio test, again finding no significant difference between the models (p = 0.24).
...
Data from 37 patients receiving a non-depleted allogenic bone marrow transplant were examined to see which variables were associated with the occurrence of acute [graft-versus-host disease](https://en.wikipedia.org/wiki/Graft-versus-host_disease) (GvHD: 0=no, 1=yes) (Bagot et al., 1988).

Possible predictors are 

-   TYPE (type of leukemia: 1=AML, [acute myeloid leukemia](https://en.wikipedia.org/wiki/Acute_myeloid_leukemia); 2=ALL, [acute lymphocytic leukaemia](https://en.wikipedia.org/wiki/Acute_lymphoblastic_leukemia); 3=CML, [chronic myeloid leukemia](https://en.wikipedia.org/wiki/Chronic_myelogenous_leukemia)),
...
m4 <- ex8_3 |> 
  select(-gvhd, -patient) |> 
  glm(gvhd.numeric ~ ., data = _ , family = "binomial")
...
**Answer**: You can set the `conf_level` argument in the `modelsummary` function to override the default 95% for the CI and set it to 90%. Running the analysis again, we find the following results:
...
ex8_3 |> 
  ...
  glm(gvhd.numeric ~ CML + preg + logind, data = _ , family = "binomial") |> 
